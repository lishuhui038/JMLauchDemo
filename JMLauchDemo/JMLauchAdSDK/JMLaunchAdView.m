...
#define kPlaceholderImage [UIImage imageNamed:@"1"] //占位图
...
@property (nonatomic, copy  ) JMLaunchAdClickBlock adClickBlock;    //广告点击

@property (nonatomic, assign) JMSkipShowType       showSkipType;

@property (nonatomic,strong ) CAShapeLayer         *shapelayer;

@end

@implementation JMLaunchAdView


- (instancetype)initWithFrame:(CGRect)frame adDuration:(NSInteger)adDuration showSkipType:(JMSkipShowType)showSkipType{
...
- (void)setAnimationSkipWithAttribute:(UIColor *)strokeColor lineWidth:(NSInteger)lineWidth  backgroundColor:(UIColor *)backgroundColor textColor:(UIColor *)textColor{
    if(self.showSkipType == JMSkipShowTypeAnimation){
...
- (void)dispath_Tiemr{
    ...
    dispatch_source_set_event_handler(_dispatchTimer, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            [self->_btnSkip setTitle:self->_showSkipType == 1?[NSString stringWithFormat:@"%ld 跳过",(long)duration]:@"跳过" forState:UIControlStateNormal];
            if(duration == 1){
...
#pragma mark - 异步加载图片
+ (instancetype)initImageWithAttribute:(NSInteger)adDuration showSkipType:(JMSkipShowType)showSkipType setLaunchAd:(JMSetLaunchAdBlock)setLaunchAd{
    static JMLaunchAdView *launchAd = nil;
...
#pragma mark - 加载广告图
- (void)setWebImageWithURL:(NSString *)strURL options:(JMWebImageOptions)options result:(JMWebImageCompletionBlock)result adClickBlock:(JMLaunchAdClickBlock)adClickBlock{
    [self addAdImgView];
    _adClickBlock = [adClickBlock copy];
    [_launchAdImgView jm_setImageWithURL:[NSURL URLWithString:strURL] placeholderImage:kPlaceholderImage options:options completed:result?result:nil];
}
...
+ (void)clearDiskCache{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString *path = [JMWebImageDownloader cacheImagePath];
        [fileManager removeItemAtPath:path error:nil];
        [JMWebImageDownloader checkDirectory:path];
    });
}
